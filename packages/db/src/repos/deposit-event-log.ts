...
type LoggedEvent = typeof depositEvents.$inferSelect;
 ...
 * that is logged, would drop an event nothing proposes again. The owner reopens the later one first.
 */
function blockedBy(event: Pick<LoggedEvent, 'kind' | 'dueOn'>, later: readonly Pick<LoggedEvent, 'kind'>[]): boolean {
  return later.length > 0 && (event.kind === 'maturity' || later.some((row) => row.kind === 'maturity'));
}

/**
 * Takes one logged event off the log and undoes what its confirm did beyond the ledger, so its proposal returns as
 * it was: a roll-over's new term is taken back, and a close re-opens the deposit with its automation on. The
 * postings themselves are the caller's (a void has already taken one back; a hand-recorded event posted none).
 * Refused with NOT_LAST, before anything is written, while a later event of the deposit is logged across a maturity.
 */
async function reopenLoggedTx(tx: Db, ws: WorkspaceContext, event: LoggedEvent): Promise<void> {
  ...
  // A roll-over logs what it replaced (prior_term_months is never null for one, always null for a close), which is
  // what tells the two apart for an event recorded by hand, whose postings are both null.
  if (event.priorTermMonths !== null) {
    // Its confirm moved the maturity on and started a term on the due day. Take that term back.
 ...
 * tax report drops it. Everything the confirm did is taken back with it (`reopenLoggedTx`). Refused (NOT_LAST, which
 * rolls the whole void back) while a later event of the deposit is logged across a maturity.
 *
...
export async function reopenDepositEventTx(tx: Db, ws: WorkspaceContext, transactionId: string): Promise<string[]> {
  ...
  if (!event) return [];
  await reopenLoggedTx(tx, ws, event);
  return [event.interestTransactionId, event.principalTransactionId].filter((id): id is string => id !== null && id !== transactionId);
}

/**
 * "Undo recorded by hand": the event goes back on the queue exactly as a void would put it back, and nothing is
 * posted or voided, since recording it by hand posted nothing. Only a hand-recorded event of this workspace; one
 * the app posted is reopened by voiding its transaction. The same NOT_LAST order holds.
 */
export async function undoRecordedByHandTx(tx: Db, ws: WorkspaceContext, eventId: string): Promise<void> {
  if (!(await automationTablesExist(tx))) throw new DepositAutomationError('NOT_READY', 'Update the app to automate a deposit');
  const [event] = await tx
    .select()
    .from(depositEvents)
    .where(and(eq(depositEvents.id, eventId), eq(depositEvents.workspaceId, ws.workspaceId)));
  if (!event) throw new DepositAutomationError('NOT_FOUND', 'That event is not recorded on this deposit');
  if (event.recordedByHand !== 1) {
    throw new DepositAutomationError('POSTED', 'The app posted this one. Delete its transaction to reopen it.');
  }
  await reopenLoggedTx(tx, ws, event);
}

export interface HandRecordedEvent {
  id: string;
  kind: LoggedEvent['kind'];
  dueOn: string;
  grossMinor: number;
  taxMinor: number;
  netMinor: number;
...
/** The deposit's hand-recorded events that can be undone now, newest first: those no later event blocks. */
export async function undoableByHandTx(tx: Db, ws: WorkspaceContext, accountId: string): Promise<HandRecordedEvent[]> {
  if (!(await automationTablesExist(tx))) return [];
  const rows = await tx
    .select()
    .from(depositEvents)
  ...
  return rows
    .filter((row) => row.recordedByHand === 1 && !blockedBy(row, rows.filter((other) => other.dueOn > row.dueOn)))
    .sort((a, b) => b.dueOn.localeCompare(a.dueOn))
    .map(({ id, kind, dueOn, grossMinor, taxMinor, netMinor }) => ({ id, kind, dueOn, grossMinor, taxMinor, netMinor }));
}

...
(Merge this import block with the one Task 2 wrote at the top of the file — `sql` and `type Db` are already imported there, and importing `Db` twice is a TypeScript duplicate-identifier error, not a lint warning.)
...
Add `import { setAsideViews } from './set-aside';` (no cycle: `set-aside.ts` imports `goals`, `ledger` and `set-aside-tx`, never `goal-funding`). Remove the now-unused `nativeBalances` import. `GoalPlanRow` gains `unconvertedWarning: string | null;`. In `goalPlansFor` replace the `over`/`warnings` lines with:
...
**Files:**
- Modify: `packages/db/src/repos/debts.ts`, `loans.ts`, `expense-templates.ts`, `statements.ts`, `drafts.ts`, `trades.ts`, `goal-transfers.ts`, `convert.ts`, `set-aside-tx.ts` (the `undoSetAsideTx` move branch, below)
- Test: `packages/db/test/set-aside-doors.test.ts`, `packages/db/test/goal-transfers.test.ts` (one new case)
...
import { beforeEach, describe, expect, it } from 'vitest';
import {
  type AccountRow, categoryIdsByKeyTx, confirmDraft, convertToPurchase, createAccount, createCardAccount, createDraft, type Database,
...
`assetKind: 'stock'` is the real listed-shares preset id (`packages/core/src/assets/presets.ts:27`: unitKind `shares`, lotSize 100, planGroup `invest`); `recordTrade` does not enforce the lot size, so one share (`unitsMicro: 1_000_000`) records. Checked in the pre-flight scan — no lookup left to do.
...
The delete names above were read from `TwoTapDelete.tsx:16` and `TransactionRow.tsx:32` in the pre-flight scan.
...
`save()` sends `{ ...post.input, ratesToBase, setAside: setAside.choice }`; `{setAside.node}` above its `ErrorBox`; its Save `disabled={busy || !setAside.ready}`. `EditSheet.tsx` line 5 imports only `useState` from `react` — add `useMemo` to it; add `useCanHold, useSetAsideChoiceOf` from `../goals/queries`, `useSetAside` from `../goals/SetAsideQuestion`, `doorOfForm` from `../goals/set-aside-question`, and `formToPost` from `./tx-form` if the file does not already import it. The `changeable` gate is untouched: a refused edit still shows its sentence and no Save.
...
- [ ] **Step 4: `PaySheet`** — door `spendingDoor(payer, typedMinor)` with `typedMinor` read by `parseMajor(amount.trim(), currency)` in a try (0 on failure) — the same parse the submit already does at line 59, lifted to render scope, not a second one; payment gains `setAside: setAside.choice`; Record disabled until ready. Record is `type="submit"`, so Enter submits too: the submit handler returns early while `!setAside.ready`.
...
**Interfaces:** Consumes `useSetAside`, `spendingDoor`, `tradeDoor`, `useSetAsideChoiceOf`, and the Task 6 inputs.

- [ ] **Step 1: Failing tests** — one per door on the laptop figures (Rp 6.800.000 out of Jenius): a loan instalment (principal 5.000.000 + interest 1.800.000), an extra payment, lending Rp 6.800.000 to Andi, repaying Rp 6.800.000 to Budi from Jenius after borrowing Rp 10.000.000 from him **into a second account (BCA)** (Jenius's free money stays Rp 5.000.000, so over by 1.800.000 — a repayment larger than what is owed is refused by `repayBorrowedPostings` with OVER_REPAYMENT, and borrowing into Jenius would raise its free money above the payment), a gold buy with no goal from Jenius, a gold buy **for Umrah** of Rp 13.000.000 (Umrah's own 7.500.000 + 5.000.000 free leaves 500.000 over; the only goal offered is the Emergency fund, and there is no second question), a purchase on an event. Each asserts the over figure, Save disabled until answered, and the shortfall figure on `/goals` after saving. Also: **AddAssetForm** pays from Opening Balances (`cashAccountId: null`) and never asks — assert adding gold with the Jenius goals in place shows no question.

- [ ] **Step 2: Wire each door** — the same four lines everywhere, plus: where a door's save control is not a `disabled`-able button (`LoanDetailPage`'s "Save payment"/"Save extra payment" rows are guarded by `!busy &&`) add `&& setAside.ready` to that guard; where the form submits on Enter (`DebtForm`, `EventDetailPage` — `<form onSubmit>`) the submit handler returns early while `!setAside.ready`. In `EventDetailPage` the `ErrorBox` sits above and outside the `<form>`: render `{setAside.node}` inside the form, directly above the Save row, and give that row `disabled={busy || !setAside.ready}` (it has no disabled today):
...
|---|---|---|
| `LoanDetailPage` payment | `filled.moneyId` | principal + interest + Σ `extras[].amountMinor`, from `paymentDraftToInput(filled, accountId, currency, balanceMinor, loanName, today)` inside a try (it throws on an incomplete draft; 0 then). Sum its fields; do not re-parse |
| `LoanDetailPage` extra | `moneyId` | `amountMinor + penaltyMinor` — `recordExtraPayment` (`loans.ts:406`) takes both out of the money account. Both are read today inline with `Number(x.replace(/\./g, ''))` (the penalty inside the save handler at line 247): lift that exact expression to render scope as `penaltyMinor` and use it in both places, unchanged — replacing it with `parseMajor` is spec §12.5, out of scope and queued. The door must measure what the repository will post |
| `DebtForm` | `draft.moneyId` | `debtDraftToInput(draft, currency, today).amountMinor` (in a try) when `draft.direction === 'lent'`; otherwise `null` door |
| `PersonCard` | per loan being repaid: `draft.moneyId \|\| moneyAccounts[0]?.id \|\| ''` — the same fallback `saveRepayment` uses | `repaymentDraftToInput(…)` for that loan (in a try) → `amountMinor + (interestMinor ?? 0)` when `person.direction === 'borrowed'` (`repayBorrowedPostings` takes both out); otherwise `null` door. The repayment input is built per loan (it needs that loan's `balanceMinor`), so the hook lives in a small per-loan component beside its Button |
| `TradeForm` | — | for a buy: `tradeDoor(draftToInput(draft, currency, today))` (Task 7) inside a try — the one buy rule, never a copy. `draftToInput` never sets `cashMinor`, so the cost is the outflow, which is what `cashLines` posts. Editing passes `excludeTransactionId: editing?.transactionId ?? null` (`TradeRow.transactionId` is nullable) and `initial` from `useSetAsideChoiceOf` |
| `EventDetailPage` | `payingWith` (the account id, line 158) | the `amount` state (line 122) parsed exactly as `record()` parses it, in `payingWith`'s currency, lifted to render scope (today `paidWith` and `amountMinor` are locals inside `record()`); door only when > 0 |
...
`recordTyped`: `const { read } = readQuick(…); if (!read) return false; const input = quickToInput(read, accounts); return guarded('typing', spendingDoor(read.accountId, outflowFrom(input.lines, read.accountId)), null, async (choice) => { await postTransaction(database, ws, { ...input, ratesToBase: await ratesFor(read), setAside: choice }); });`. `saveRecorded` the same with `replaceTransaction(database, ws, id, …)`, `exclude = id`, and `close()` at the end of its `save`. `saveDraft` with `record` true: its account and amount come from `draftFields(values)` (`accountId`, `amountMinor` — there is no draft object in scope), door `spendingDoor(fields.accountId ?? '', Math.max(0, fields.amountMinor))`, and `save` runs `editDraft(…)` then `confirmDraft(database, ws, id, { setAside: choice })` then `close()`; with `record` false it stays a plain `run`. Render `{asking && <SetAsideSheet door={asking.door} excludeTransactionId={asking.exclude} onSave={async (choice) => { await asking.save(choice); await invalidate(); asking.settle(true); }} onClose={() => { asking.settle(false); setAsking(null); }} />}` — `SetAsideSheet` calls `onClose` after a successful save too, and a promise that already settled true ignores the later false. `TransactionRow.fileAs` is **not** changed: it re-files a category, passes no `setAside`, and the ledger carries the answer (spec §4.4).

- [ ] **Step 3: Review queue** — the Record glyph (today `run(draft.id, () => confirmDraft(database, ws, draft.id))`, lines 125–133) asks `asksAboutSetAside(database, ws, spendingDoor(draft.accountId ?? '', Math.max(0, draft.amountMinor)))` first: silent → `run(draft.id, () => confirmDraft(database, ws, draft.id, { setAside: null }))` as today; asking → the same `SetAsideSheet` state as `TransactionsPage`, whose `onSave` runs `confirmDraft(…, { setAside: choice })`. `ReviewPage`'s own `run` returns void, so it needs no settle.
...
Replace the old `passes on a set-aside amount above the balance` test (line 124) with the second one above. `formatMinor(…, 'IDR')` was checked in the pre-flight scan: `Rp` + U+00A0 + digits; `US$100,03` has no space.

- [ ] **Step 2: `overview-rows.ts`** — `AttentionItem` (line 17) today is `{ key; tone; text; action; to }` with `to` a union of five routes: add `'/net-worth/assets/$accountId'` to the union and `params?: { accountId: string }`. `attentionItems` takes six parameters today; `shortAccounts: AccountSetAsideRow[] = []` is the seventh. The goal loop (line 52) reads `plan.unconvertedWarning` where it read `plan.earmarkWarning`; append:
...
`OverviewPage` passes `Object.values(useSetAsideViews().data ?? {})` as the seventh argument (the call at line 127) and `params={item.params}` on the attention `InsetRow` (lines 218–225; `InsetRow.params` is `LinkProps['params']`).

- [ ] **Step 3: Lift `ShareBar`** out of `OverviewPage.tsx` (today `ShareBar({ groups: SheetGroup[]; totalMinor })` at line 30, returning null when `totalMinor <= 0`) into `ShareBar.tsx`, unchanged in look, taking `segments` instead of `groups` (`OverviewPage` maps its groups to `{ key, label, minor: group.totalMinor, className: GROUP_COLORS[group.key] ?? 'bg-slate-400' }`). The legend is inline markup inside `SheetColumn` (lines 58–65) today, not a component: extract it as `ShareLegend({ segments, totalMinor })` and have `SheetColumn` render it, so the Overview looks exactly as before.
...
In `AssetDetailPage`, render `<SetAsidePanel accountId={accountId} />` inside the `{value && (<> … </>)}` fragment (lines 73–113), directly after `<Hero …/>` and before the market form (line 97) — the forms live inside that same fragment, so "after the fragment" would put the panel below them.

- [ ] **Step 5: e2e** — in `set-aside.spec.ts`: after a Rp 21.500.000 transfer out of Jenius to a new BCA account answered **borrowing** from Umrah — then a second Rp 100.000 expense asks nothing (already short); the account page shows "You have promised more than this account holds", Free to spend at minus 16.500.000 (as `Money` prints a negative) in alarm, Umrah "Short by Rp 7.500.000" (it borrowed 16.500.000 but can carry no more than it promised) and the Emergency fund "Short by Rp 9.000.000" (the rest, by rank) — figures asserted, not the labels alone (match `Rp` then any one character: formatMinor uses U+00A0); Net worth's Needs attention has exactly one Jenius row and it opens the account page. `phone-set-aside.spec.ts` repeats the laptop test on the `phone` project through the keypad (`typeAmount` handles it) and checks the account page in dark mode (`page.emulateMedia({ colorScheme: 'dark' })`) has no text on a literal white.
...
(`plan(partial)` at line 23 and `goal(partial)` at line 7 are the file's fixture helpers; there is no `goalRowOf`.) `fundedWindow` returns the window only when the newest `borrowed` entry exists; `from` is a `reached` entry dated on or before it, else null. It is shown only while the goal has a short line.

- [ ] **Step 2: `goal-cards.ts`** — `done = plan.goal.kind !== 'emergency' && plan.stages.length > 0 && plan.stages.every((stage) => stage.state === 'paid')` (ledger Ruling Q3: an emergency fund is a standing level and never reads Done, even with a stage marked paid by hand; `applySetAsideTx` never marks one paid); `statusLabel = done ? 'Done' : …existing`; `statusTone = done ? 'good' : …existing`; `doneOn` = the latest `paidOn` of `plan.goal.stages` when `done`, else null; `shortLines` from `plan.links.filter((link) => link.shortMinor > 0)`; export `fundedWindow`.

- [ ] **Step 3: `GoalsPage`** — per card, under the hero caption: each short line as `Short by <Money>` in `var(--ph-warn)`; when `fundedWindow(history[goalId])` exists and the card has a short line: `Fully funded 3 Aug – 19 Sep` (or `Fully funded until 19 Sep`), then `<Money> went to <description>. Put it back and the fund is complete again.` in `var(--ph-ink-3)`. The Funded-by row's subtitle becomes `set aside · short by <Money>` when `link.shortMinor > 0`. A `History` `InsetGroup wide` lists `useGoalHistory().data?.[goalId]` rows (title by kind: "Set aside", "Taken back", "Borrowed", "Spent", "Moved", "Reached the target"; subtitle `date · text`; value the signed `Money`, none for reached). The archive group (lines 290–292, `<InsetGroup wide><DestructiveRow label="Archive" …/></InsetGroup>`, no footer today) gains `footer="Keeps the history, stops it claiming money."` when `card.done`. GoalsPage has no day-and-month formatter today (the `en-GB` helpers are `monthYear` in goal-cards.ts and `shortDate` in overview-rows.ts, both with a year): add `dayMonth(iso) = new Date(`${iso}T00:00:00Z`).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', timeZone: 'UTC' })` to `goal-cards.ts` beside `monthYear`, with a unit test (`dayMonth('2026-08-03')` is `'3 Aug'`), and use it for "3 Aug – 19 Sep".

- [ ] **Step 4: `GoalForm` free hint** — `const views = useSetAsideViews().data ?? {}; const balances = useBalances().data ?? {};` (`useBalances` from `../../lib/queries`, not imported in GoalForm today; add `formatMinor` to its `@expanses/core` import — there is no `balanceOf` in the file). For each box, `const room = (views[account.id]?.freeMinor ?? balances[account.id] ?? 0) + (earmarkOf(account.id)?.amountMinor ?? 0)` — the free money plus this goal's own promise there, which the save replaces. The field's `hint` reads `${formatMinor(Math.max(0, room), currencyOf(account))} free for this goal`; when the typed figure, read with `parseMajor(typed, currencyOf(account))` inside a try, is more than `room`, it reads (warn) `That leaves ${account.name} ${formatMinor(typed - room, currencyOf(account))} short`. Where no view exists (nothing promised on the account), `room` is its balance from `useBalances()`. The box is `<Field key={account.id} label={`${account.name} (${currencyOf(account)})`}>` at line 205; `Field` takes `hint?: ReactNode`. **The parse keeps `currencyOf(account)`** — the foreign-box test in `goals.spec.ts` must still pass.
...
| Axis | Values |
|---|---|
| door | expense · transfer to BCA (IDR) · transfer to Wise (cross-currency, landed 123.465 cents for Rp 20.000.000 — with 123.463 the well-over row moves 92.597,25 cents, a .25 where floor and round agree) · transfer to a card · tagged transfer for Umrah · tagged buy for Umrah · untagged buy · bill · loan instalment |
...
Rules the table must show (write each as its own `expect`, not a label): nothing-promised and within-free rows are `silent` and write no draw; already-short rows are `already-short` and write no draw; a card destination never offers `move`; a tagged transfer or buy never offers `spend`; `spend` lowers the promise by the whole payment up to it; `spend` from the Emergency fund (kind `emergency` in this fixture) marks no stage paid and `goalCard` does not read Done, while `spend` from a one-stage non-emergency goal marks its stage paid and reads Done (ruling Q3); `move` across currencies lands `movedAmount(...)`; the USD rows' figures are cents on the USD account and no IDR figure changes.
...
- [ ] **Step 2: End-to-end walk** — `set-aside-combinations.spec.ts`, chromium and phone, amounts typed per keystroke, walking these journeys end to end and asserting the figures on the account page and on `/goals` after each:
  ...
  4. Transfer Rp 20.000.000 to Wise with Received US$1.234,65 → move EF: the question moves what is over the free money, Rp 15.000.000, so Wise shows US$925,98 set aside — 15.000.000 × 123.465 ÷ 20.000.000 = 92.598,75 cents, floored (rounding would show US$925,99; the old figure US$111,11 was the Rp 1.800.000 of the repository test, not what this journey moves) — Jenius's EF promise is Rp 15.000.000, and the goal counts the dollars converted or names them unconverted exactly as the foreign set-aside test does.
  5. Tagged transfer Rp 7.500.000 for Umrah to BCA: silent; Umrah's promise now on BCA; Jenius free still Rp 5.000.000.
  6. Umrah tickets of Rp 6.000.000 (Rp 1.000.000 over the free Rp 5.000.000) → Yes → Umrah's promise 7.500.000 − 6.000.000 = Rp 1.500.000 (taking only the part over what was free would leave Rp 6.500.000) → Done → Archive → Jenius free rises from Rp 5.000.000 to Rp 6.500.000, by the Rp 1.500.000 Umrah still promised. (Paying the full 7.500.000 would leave nothing promised, and "free rises by nought" cannot fail.)
  ...
  9. (Ruling Q3, if the goal form can create a kind-`emergency` goal with a fixed amount; otherwise this lives in the repository matrix only) Spend Rp 6.800.000 from an emergency-kind fund with "Yes — this is what I saved for": the fund's set-aside falls by 6.800.000, it does **not** read Done, and no archive note shows.
...
| §2.7 / §4.4 | borrow / spend / done; the emergency fund is a standing level that never reads Done (ledger Ruling Q3, overturning §11.3's default) | 3, 13, 14 |

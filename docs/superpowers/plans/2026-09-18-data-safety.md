...
- **Never destroy user data on any path.** No code in this plan may delete, overwrite, wipe or reset anything except behind an explicit, repeated user press. Every destructive path that leaves the data on the device takes a snapshot first — a restore takes a `before-restore` copy, a migration a `before-migration` one. **Amended 2026-09-19:** Start fresh is the exception, and it is deliberate. It removes both `.expanses/` and `expanses-safety/`, because a clean slate that quietly kept two copies of the data the user just asked to be rid of would be a lie — so a `before-start-fresh` snapshot would be deleted by the same press that took it, and none is written. What guards that path instead is the dialog: two presses, with the red button disabled until the user has downloaded a backup there or ticked that they already hold one. `pool.wipeFiles()` and `removeVfs()` still appear in exactly one place.

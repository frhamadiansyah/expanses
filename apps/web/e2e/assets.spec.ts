...
async function addBank(page: Page) {
  ...
  await page.getByLabel('Opening balance').fill('50000000');
...
test('records past purchases without touching the bank balance', async ({ page }) => {
  ...
  await expect(page.getByText('Rp 22.400.000').first()).toBeVisible();
  await expect(page.getByText('Rp 50.000.000').first()).toBeVisible();
});
...
test('buys, sells, and works out the gain at average cost', async ({ page }) => {
  ...
  await expect(page.getByText('Rp 26.380.000').first()).toBeVisible();
...
test('updates a price from the assets list and shows the new value', async ({ page }) => {
  ...
  await expect(page.getByText('Rp 27.630.000').first()).toBeVisible();
});
...
test('sends trade edits to Buy & sell instead of the transaction form', async ({ page }) => {
  ...
  await expect(page.getByText('Bought 10 Antam gold bars')).toBeVisible();
  await expect(page.getByRole('link', { name: 'Buy & sell' }).first()).toBeVisible();
});

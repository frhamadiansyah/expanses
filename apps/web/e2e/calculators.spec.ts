...
test('answers what retirement costs a month without saving anything', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Dana hari tua' })).toHaveCount(0);
});
...
test('turns the answer into a goal, which reaches the budget sheet', async ({ page }) => {
  ...
  await page.getByLabel('Your age now').fill('35');
  await page.getByLabel('Age you retire').fill('55');
  await page.getByRole('button', { name: 'Save Dana hari tua as a goal' }).click();
  await expect(page.getByText('Saved Dana hari tua as a goal.')).toBeVisible();

  await page.goto('/net-worth/goals');
  await expect(page.getByRole('heading', { name: 'Dana hari tua' })).toBeVisible();
  await expect(page.getByText('Worked out from your figures')).toBeVisible();

  await page.goto('/budget');
  await expect(page.getByTestId('savings-Dana hari tua')).toContainText('a month');
});

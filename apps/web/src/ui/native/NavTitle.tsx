    ...
    <header className="mb-[14px] md:max-w-4xl">
      {back && (
        <button
          type="button"
          onClick={onBack}
          className="ph-focus mb-[2px] -ml-[2px] inline-flex min-h-[28px] items-center rounded px-[2px] text-[14px] leading-[18px] text-[var(--ph-tint)]"
        >
          {backLabel(back)}
        </button>
      )}
        ...
        {(plan.inline.length > 0 || plan.overflow.length > 0) && (
          <div className="flex shrink-0 items-center gap-[8px]">
            {plan.inline.map((action) => (
              <CornerButton key={action.key} label={action.label} onClick={() => action.run?.()} destructive={action.destructive}>
                {action.glyph}
              </CornerButton>
            ))}

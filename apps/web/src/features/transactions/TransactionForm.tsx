 ...
 *
 * `keep` is what the field already holds, which is offered whatever it is: a transaction recorded years ago
 * against a property account must open showing that account, not an empty box.
 */
        ...
        <div className="grid gap-3 md:grid-cols-2" hidden={buying}>
          ...
          <Field label={draft.mode === 'expense' ? 'Paid with' : draft.mode === 'income' ? 'Received into' : 'From'}>
            <Select value={draft.moneyId} onChange={(e) => set({ moneyId: e.target.value })}>
              <MoneyAccountOptions accounts={accounts} spendableOnly={draft.mode !== 'transfer'} keep={draft.moneyId} />
            </Select>
          </Field>
          {askWhichCard && (

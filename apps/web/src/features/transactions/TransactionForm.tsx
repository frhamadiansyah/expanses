...
export function TransactionForm({
  ...
  const [draft, setDraft] = useState<Draft>(() =>
    initial ? draftFromTransaction(initial) : emptyDraft(),
  );
  ...
  const crossCurrency =
    ...
    !!moneyAccount &&
    !!toAccount &&
    moneyAccount.currency !== toAccount.currency;
  const onCard =
    draft.mode === "expense" && moneyAccount?.subtype === "credit_card";
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const foreign = [...new Set(lines.map((l) => l.currency))].filter(
        (c) => c !== ws.baseCurrency,
      );
      ...
      if (resolved.missing.length > 0) {
        ...
        throw new Error(
          `No ${resolved.missing[0]}→${ws.baseCurrency} rate for ${rateDate}. Enter it below.`,
        );
      }
      const input = {
        occurredOn: draft.occurredOn,
        description:
          draft.description || (draft.mode === "transfer" ? "Transfer" : ""),
        lines,
      ...
      <form onSubmit={submit} className="space-y-3">
        <div className="flex gap-2">
          {(["expense", "income", "transfer"] as const).map((m) => (
            <Button
              key={m}
              variant={draft.mode === m ? "primary" : "secondary"}
              aria-pressed={draft.mode === m}
              onClick={() => set({ mode: m, categoryId: "", splits: [] })}
            >
              {m === "expense"
                ? "Expense"
                : m === "income"
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          >
            <Select
              value={draft.moneyId}
              onChange={(e) => set({ moneyId: e.target.value })}
            >
          ...
          {draft.mode === "transfer" ? (
            <Field
              label="To"
              hint="Paying a credit card bill? Choose the card here."
            >
              <Select
                value={draft.toId}
            ...
            {showOriginal && (
              <div className="mt-2 grid gap-3 md:grid-cols-2">
                <Field
                  ...
                  hint="The currency the merchant charged. Some cards earn more in certain currencies."
                >
                  <Select
                ...
                <Field
                  label={`Original amount${draft.originalCurrency ? ` (${draft.originalCurrency})` : ""}`}
                >
                  <Input
            ...
            {draft.splits.map((row, i) => (
              <div key={i} className="grid grid-cols-[1fr_8rem_auto] gap-2">
                <Select
                  ...
                  onChange={(e) =>
                    set({
                      splits: draft.splits.map((r, j) =>
                        j === i ? { ...r, categoryId: e.target.value } : r,
                      ),
                    })
                  }
                >
                ...
                <Button
                  variant="ghost"
                  onClick={() =>
                    set({ splits: draft.splits.filter((_, j) => j !== i) })
                  }
        ...
        {needsRate && (
          <Field
            label={`Rate: ${ws.baseCurrency} per 1 ${needsRate}`}
            hint={
              ratePreview(manualRate, needsRate, ws.baseCurrency) ??
              "Type the rate your bank used."
            }
          >
            <Input
              value={manualRate}

...
/**
 * What life cover opens on. Debts and liquid assets come from the balance sheet, through `ratioTotals` — the reader the
 * net-worth page uses — and are null when a row's currency has no rate yet (the sheet's own `missing`): a figure
 * missing an account is flagged, never passed off as the whole.
 */
...
export function lifeCoverPrefill(
  ...
  const known = totals !== null && missing.length === 0;
  return {
...
export function lifeCoverWorking(draft: LifeCoverDraft, prefill: LifeCoverPrefill, currency: string): LifeCoverWorking {
  ...
  const noRate = `No rate for ${prefill.missing.join(', ')} yet, so the balance sheet cannot count those accounts. Type the figure.`;

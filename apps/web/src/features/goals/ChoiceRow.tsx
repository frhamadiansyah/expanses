...
import { type GroupChild, InsetGroup, PickerRow } from '../../ui/native';
    ...
    <>
      ...
      {anchor && (
        <ChoiceMenu label={label} anchor={anchor} onClose={() => setAnchor(null)}>
          {options.map((option) => (
            /* The platform's own menu: larger type, and the tick leading the one in force. */
            <button
              key={option.value}
              type="button"
              onClick={() => {
  ...
  return (
    <div className="fixed inset-0 z-30 bg-[var(--ph-scrim)]" onClick={onClose} role="presentation">
      <div
        ...
        className="absolute w-fit max-w-[calc(100%-1.5rem)] rounded-2xl bg-[var(--ph-surface)] p-1.5 text-[var(--ph-ink)] shadow-xl outline-none"
